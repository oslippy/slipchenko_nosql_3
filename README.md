...
Результат (усі рядки — їх лише 6):

| movie | rating_u1 | rating_u2 |
|--|--|--|
| Awakenings (1990) | 5 | 4 |
...
| recommendation | recommended_by | avg_rating |
|--|--|--|
| Shawshank Redemption, The (1994) | 47 | 4.7 |
| Silence of the Lambs, The (1991) | 46 | 4.83 |
| Raiders of the Lost Ark (1981) | 46 | 4.7 |
| Star Wars: Episode V - The Empire Strikes Back (1980) | 45 | 4.69 |
| When Harry Met Sally... (1989) | 45 | 4.6 |
| Star Wars: Episode VI - Return of the Jedi (1983) | 45 | 4.47 |
| It's a Wonderful Life (1946) | 43 | 4.91 |
| Shakespeare in Love (1998) | 43 | 4.63 |
| Indiana Jones and the Last Crusade (1989) | 43 | 4.42 |
| As Good As It Gets (1997) | 43 | 4.42 |
...
| hops | chain |
|--|--|
| 2 | User#1 → Awakenings (1990) → User#2 |
...
Ось як супервузол виглядає очима Neo4j Browser — American Beauty в центрі й сотні глядачів-променів навколо. Browser намалював лише перші 300 ребер із 3428, і вже те, що йому довелося обрізати показ, добре показує проблему.
...
Запити — у [`queries/part5_gds.cypher`](queries/part5_gds.cypher). GDS не працює зі збереженим графом напряму — спершу будуємо проєкцію в пам'ять (`gds.graph.project`), ганяємо алгоритм, потім проєкцію видаляємо. Тимчасові ребра (`CO_RATED`, `SIMILAR`) теж матеріалізуємо перед проєкцією й прибираємо після.

Одне практичне зауваження. Матеріалізація пар користувач-користувач на порозі `>= 4` не вкладається в пам'ять транзакції — пар користувачів на порядки більше, ніж пар фільмів. Тож для 5.2 і 5.3 я підняв поріг до `= 5`, як і пропонує умова. Граф фільмів (5.1) на `>= 4` рахується нормально.
...
Граф `movieGraph` з'єднує фільми ребром `CO_RATED`, якщо їх обох високо оцінив один користувач (вага — скільки таких користувачів). PageRank шукає фільми, на які веде багато «голосів» від інших добре зв'язаних фільмів. `gds.util.asNode` дістає вузол за GDS-ідентифікатором, щоб дібратися до назви. Колонку `num_ratings` виводжу навмисно — щоб порівняти ранг PageRank із простою кількістю оцінок.

Топ очолюють American Beauty (PR 9.66) і Star Wars. Цікаве починається нижче. Fargo з 2513 оцінками обганяє за PageRank і Matrix (2590 оцінок), і Saving Private Ryan (2653), і Return of the Jedi (2883) — хоча оцінок у нього менше. Те саме з Godfather (2223) і Shawshank (2227), що лізуть угору повз популярніші фільми.

### Що означає високий PageRank — це просто популярний фільм?

Ні, не просто. Кількість оцінок — це лише степінь вузла, а PageRank зважає ще й на те, з ким фільм зв'язаний. Високий бал дістається тому, кого оцінюють разом з іншими центральними фільмами. Тобто це радше «фільм-консенсус», який люблять любителі хорошого кіно загалом, а не просто касовий хіт. Тому Fargo й Godfather і обходять кілька стрічок, що зібрали більше оцінок, — їх співоцінюють у хорошій компанії.
...
Граф `userSimilarity` з'єднує користувачів ребром `SIMILAR`, якщо вони обидва поставили 5 тим самим фільмам (вага — кількість таких фільмів). Louvain знаходить у ньому спільноти й записує номер кожному користувачу у властивість `community`. Далі двома запитами беру 10 найбільших спільнот і три найпопулярніші жанри в кожній (за фільмами, які її учасники оцінили на 4+).

Результат чесно неоднозначний. `modularity` вийшла лише 0.17 — розбиття слабке, і реально великих кластерів усього чотири (приблизно 760, 300, 280 і 120 користувачів). Решта 4570 — одинаки, що не потрапили в топ-50000 найсильніших ребер. Це закономірно — активні глядачі перетинаються на тих самих культових фільмах, тож на чіткі «племена» не діляться.

Сирий топ-3 жанрів це підтверджує — у всіх чотирьох кластерах нагорі Drama, Comedy й Action, бо ці жанри домінують у датасеті взагалі. Щоб побачити справжню різницю, рахую `lift` — частку жанру в кластері, поділену на його глобальну частку (понад 1 означає, що жанр над-представлений). Після нормалізації виходить чотири профілі:
- сімейно-блокбастерний (~280 користувачів) — над-представлені Children's, Action, Adventure, Fantasy (×1.2–1.3);
- класичне ретро-кіно (~300) — Film-Noir ×1.7, далі Documentary, Musical, Western;
- документально-кримінальний (~120) — Documentary ×1.8, Crime, Horror;
- всеїдний центр (~760, найбільший) — усі жанри близько ×1, без вираженого нахилу.

Конкретних номерів спільнот не наводжу — Louvain недетермінований, тож id і точні розміри злегка змінюються від запуску до запуску, а смакові профілі лишаються стабільними.

Ось ці спільноти на вибірці — по ~30 користувачів із трьох найбільших кластерів цього запуску, кожен своїм кольором. Видно радше тенденцію, ніж чіткі острови — дві групи тяжіють до протилежних країв, а найбільший кластер сидить посередині й зшиває їх. Сама ця щільність перетину й відповідає низькій modularity.

![Спільноти користувачів за Louvain: три найбільші кластери, кожен своїм кольором](docs/img/communities.png)

### 1. Чи відповідають кластери інтуїтивним групам?

Частково. За сирим топом жанрів — ні, він у всіх однаковий. Але після нормалізації нахили цілком впізнавані: сімейні блокбастери, цінителі класики, документалісти й всеїдний центр. Групи є, просто слабкі — низька modularity це не випадковість, а наслідок того, що всі перетинаються на культовому кіно.

### 2. Як я це перевірив?

Двома зрізами. Спершу сирий топ-3 жанрів на кластер — він показав, що цього замало (скрізь Drama/Comedy). Потім lift проти глобального розподілу, який і витяг над-представлені жанри. Саме другий зріз дає змістовну відповідь.
...
Дейкстру запускаю на тому ж графі схожості. Вагу свідомо не передаю — `weight` тут це кількість спільних фільмів, тобто близькість, а не відстань. Якби я згодував її як вартість, Дейкстра уникала б найсильніших зв'язків (бо в них «довжина» найбільша), а для пошуку короткого ланцюжка це безглуздо. Тому ребра рахую як одиничні, і `totalCost` дорівнює кількості хопів. Другий запит від кількох джерел рахує середню й максимальну довжину шляху до всіх досяжних користувачів.

### 1. Наскільки тісний світ?

Дуже. На перевірених парах майже всюди 2 хопи, зрідка 3. Ба більше — від кожного перевіреного джерела всі 1469 інших зв'язаних користувачів досяжні щонайбільше за 3–4 кроки.

### 2. Середня довжина шляху і «шість рукостискань»

Середня — близько 2.0 (1.8–2.05 від різних джерел), максимум 3–4. Гіпотезу шести рукостискань це підтверджує з запасом — тут вистачає двох. Але чесна засторога — це не весь датасет, а проріджений граф схожості. Топ-50000 найсильніших 5-зіркових зв'язків стягують 1470 найактивніших глядачів у щільне ядро, де така тіснота закладена самою побудовою. Решта 4570 користувачів у цьому графі взагалі ізольовані. Тобто «тісний світ» тут — про ядро ентузіастів, а не про всю спільноту. Хоча й на повному графі оцінок двоє будь-яких користувачів сходяться за 2 кроки через популярні фільми (Запит 6 Частини 3).
...
Найважче в SQL було б написати Запит 6 — найкоротший ланцюжок між двома користувачами. Заковика в тому, що довжина шляху наперед невідома — може бути 2 хопи, а може 6, тож фіксованим набором JOIN-ів його не виразиш: скільки разів зджойнити `ratings` саму з собою, заздалегідь не скажеш. У SQL це лягає на рекурсивний CTE, приблизно так:

```sql
WITH RECURSIVE
links AS (                       -- двоє користувачів пов'язані, якщо мають спільний фільм
  SELECT DISTINCT a.user_id AS u1, b.user_id AS u2
  FROM ratings a
  JOIN ratings b ON a.movie_id = b.movie_id AND a.user_id <> b.user_id
),
reach AS (
  SELECT 1 AS u, 0 AS depth
  UNION ALL
  SELECT l.u2, reach.depth + 1
  FROM reach JOIN links l ON l.u1 = reach.u
  WHERE reach.depth < 10
)
SELECT min(depth) FROM reach WHERE u = 2;
```

І це ще оптимістично. Проміжна таблиця `links` — це всі пари користувачів зі спільним фільмом; на нашому датасеті сама лише American Beauty з тисячами глядачів дає мільйони таких пар (та сама проблема супервузлів із частини 4). До того ж ця рекурсія не стежить за відвіданими, тож ходить по колу й роздувається — щоб зробити її коректною і знайти саме найкоротший шлях, коду стане ще більше. У Cypher те саме — один рядок `shortestPath((a)-[:RATED*..10]-(b))`, бо обхід змінної довжини й пошук найкоротшого це рідна задача графа, а не побічний трюк. Запит 5 (рекомендації через схожих) має ту саму природу — у графі це звичайний дво-хоповий патерн, а в SQL кілька самоз'єднань `ratings` із підзапитом «чого користувач ще не бачив», що швидко стає нечитабельним.

## 2. Де граф програє

Граф сильний у локальних обходах, але програє на масовій агрегації по всьому датасету й на табличних звітах. Коли задача — порахувати щось по всіх даних одразу (середній рейтинг за жанром, розподіл оцінок за місяцями, демографічний звіт на кшталт «середня оцінка за віком і статтю»), це по суті `GROUP BY` по мільйону рядків, і реляційна СУБД зі стовпцевим зберіганням чи матеріалізованими представленнями зробить це швидше й природніше. Наш Запит 4 (середнє по жанрах) це й показує — у Neo4j він чесно проходить усі 1М ребер RATED, тоді як у зірковій схемі (факт-таблиця `ratings` плюс виміри) це звичайний агрегат, на який заточені і сама БД, і BI-інструменти. Сюди ж і експорт даних — вивантажити «всі оцінки пласкою таблицею» в CSV для звіту чи Excel у SQL тривіально, а в графі плаский експорт радше йде проти моделі. І суто табличні дані в графі дорожчі за зберіганням — кожне з мільйона ребер несе накладні витрати, яких у простому рядку таблиці немає.

## 3. Покращення схеми

Кілька запитів частини 3 щоразу перераховують те, що можна було б порахувати один раз. Два конкретні покращення.

Перше — денормалізувати агрегати фільму. Запити 1 (трилери із середнім > 4) і 4 (середнє по жанрах) щоразу проходять усі ребра RATED, аби порахувати середній рейтинг. Якщо тримати на кожному вузлі Movie готові `avgRating` і `numRatings` (порахувати раз під час завантаження й оновлювати періодично), Запит 1 стає простим «знайди трилери, де `m.avgRating > 4`» — без обходу мільйона оцінок узагалі, а Запит 4 агрегує вже готові середні по фільмах, а не сирі оцінки. Ціна — це денормалізація, такі поля треба тримати в актуальному стані.

Друге — індекс на властивості ребра `RATED.rating`. Запит 2 (користувачі з понад 50 п'ятірками) спершу відбирає всі ребра з оцінкою 5, перебираючи їх поспіль. Neo4j 5 уміє індекси на властивостях ребер, тож індекс на `rating` дав би брати потрібні ребра одразу, а не сканувати кожне; той самий індекс пришвидшив би фільтри за рейтингом у Запитах 1 і 4. Для Запиту 2 можна піти ще далі й тримати лічильник п'ятірок прямо на вузлі User — тоді запит стане «знайди користувачів, де `fiveStars > 50`» без жодного обходу.
